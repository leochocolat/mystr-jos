	...
	_createTexture: function ( buffers, config ) {
		...
		var texturePending = this._allocateWorker( taskCost )
			...
			.then( ( message ) => {
				...
				var { mipmaps, cubeImages, width, height, format } = message;
...
BasisTextureLoader.BasisWorker = function () {
			...
			case 'transcode':
				transcoderPending.then( () => {
					...
					try {
						...
						var { width, height, hasAlpha, mipmaps, cubeImages, format } = message.taskConfig.lowLevel
	...
	function transcode( buffer ) {
		...
		var numImages = basisFile.getNumImages();
		...
		// Cubemaps
			...
			for ( var i = 0; i < numImages; i++ ) {
				...
				var cubeImageWidth = basisFile.getImageWidth( i, mip );
				var cubeImageHeight = basisFile.getImageHeight( i, mip );
				var dst = new Uint8Array( basisFile.getImageTranscodedSizeInBytes( i, mip, transcoderFormat ) );
